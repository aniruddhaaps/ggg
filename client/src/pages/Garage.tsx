...
import { loadCareerSaves, purchaseCar } from '../utils/careerManager'
...
function Garage() {
  ...
  const [ownedCars, setOwnedCars] = useState<number[]>([1, 5])
  ...
  useEffect(() => {
    ...
    const locationState = location.state as { playerData?: CareerProgress, wonRace?: boolean }
    const locationPlayerData = locationState?.playerData
    const wonRace = locationState?.wonRace
    console.log('[GARAGE DEBUG] locationPlayerData:', locationPlayerData)
    console.log('[GARAGE DEBUG] wonRace:', wonRace)
    ...
    if (locationPlayerData && locationPlayerData.username) {
      ...
      // Check if player just won career/1 race - show reward message
      if (wonRace && player && player.completedLevels.includes(1)) {
      ...
      else if (player && player.completedLevels.includes(0) && player.currentLevel === 1 && !wonRace) {
    ...
    if (player) {
      ...
      setBtcBalance(player.btcBalance || 0)
      setOwnedCars(player.ownedCars || [1, 5])
      setCompletedLevels(player.completedLevels || [])
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            {(totalCoins + btcBalance) >= 1000 ? `${((totalCoins + btcBalance) / 1000).toFixed(2)}K` : (totalCoins + btcBalance)}
          </span>
        </div>
        <div style={{
          width: '1px',
      ...
      <button
        disabled={
          currentCarIndex !== 4 &&
          !(currentCarIndex === 2 && completedLevels.includes(1))
        }
        style={{
          position: 'absolute',
          bottom: '70px',
          right: '70px',
          backgroundColor:
            currentCarIndex === 4 ? 'rgba(0, 0, 0, 0.5)' :
            (currentCarIndex === 2 && completedLevels.includes(1)) ? 'rgba(0, 0, 0, 0.5)' :
            'rgba(60, 20, 20, 0.85)',
          color:
            currentCarIndex === 4 ? '#ffffff' :
            (currentCarIndex === 2 && completedLevels.includes(1)) ? '#ffffff' :
            '#ff6b6b',
          border:
            currentCarIndex === 4 ? 'none' :
            (currentCarIndex === 2 && completedLevels.includes(1)) ? 'none' :
          ...
          cursor:
            currentCarIndex === 4 ? 'pointer' :
            (currentCarIndex === 2 && completedLevels.includes(1)) ? 'pointer' :
            'not-allowed',
          transition: 'all 0.3s ease',
          textTransform: 'uppercase',
          backdropFilter: 'blur(10px)',
          WebkitBackdropFilter: 'blur(10px)',
          boxShadow:
            currentCarIndex === 4 ? '0 4px 6px rgba(0, 0, 0, 0.1)' :
            (currentCarIndex === 2 && completedLevels.includes(1)) ? '0 4px 6px rgba(0, 0, 0, 0.1)' :
            '0 4px 12px rgba(255, 107, 107, 0.3)',
          zIndex: 10,
          opacity: 1,
        }}
        onMouseEnter={(e) => {
          if (currentCarIndex === 4 || (currentCarIndex === 2 && completedLevels.includes(1))) {
            e.currentTarget.style.backgroundColor = 'rgba(255, 255, 255, 0.9)'
            e.currentTarget.style.color = '#000000'
          }
        }}
        ...
        onClick={() => {
          ...
          // Handle Car 5 (index 4) - navigate to career/1
          if (currentCarIndex === 4 && ownedCars.includes(5)) {
          ...
          // Handle Car 3 (index 2) - purchase or select for career/2
          else if (currentCarIndex === 2) {
            if (!ownedCars.includes(3)) {
              // Purchase Car 3 for 200 BTC
              const saves = loadCareerSaves()
              if (saves.players.length > 0) {
                const mostRecentPlayer = saves.players.reduce((latest, current) =>
                  current.timestamp > latest.timestamp ? current : latest
                )
                if (purchaseCar(mostRecentPlayer.username, 3, 200)) {
                  console.log('Car 3 purchased!')
                  // Reload player data
                  const updatedSaves = loadCareerSaves()
                  const updatedPlayer = updatedSaves.players.find(p => p.username === mostRecentPlayer.username)
                  if (updatedPlayer) {
                    setBtcBalance(updatedPlayer.btcBalance || 0)
                    setOwnedCars(updatedPlayer.ownedCars || [1, 5])
                    setCompletedLevels(updatedPlayer.completedLevels || [])
                  }
                } else {
                  console.log('Not enough BTC to purchase Car 3')
                }
              }
            } else {
              // Car 3 owned - navigate to career/2
              console.log('Car 3 selected, navigating to career/2')
              const saves = loadCareerSaves()
              if (saves.players.length > 0) {
                const mostRecentPlayer = saves.players.reduce((latest, current) =>
                  current.timestamp > latest.timestamp ? current : latest
                )
                navigate('/career/2', { state: { playerData: mostRecentPlayer } })
              }
            }
          }
        }}
      >
        {currentCarIndex === 4 && ownedCars.includes(5) ? 'SELECT' :
         currentCarIndex === 2 && !ownedCars.includes(3) ? `BUY (200 BTC)` :
         currentCarIndex === 2 && ownedCars.includes(3) ? 'SELECT' : 'LOCKED'}
      </button>
      ...
      {/* Win reward message after winning career/1 */}
      {showWinReward && (
        <div style={{
          position: 'absolute',
          bottom: '80px',
          ...
          <p style={{
            ...
            You've been rewarded <span style={{ color: '#ffcc00', fontWeight: 'bold', fontSize: '28px' }}>200 BTC</span> for claiming your first blacklist win!
            <br />
            You can now <span style={{ color: '#4fc3f7', fontWeight: 'bold' }}>mine runes</span> and purchase <span style={{ color: '#ff00ff', fontWeight: 'bold' }}>Car 3</span> to continue your journey!
          </p>
        </div>
      )}
    </div>
