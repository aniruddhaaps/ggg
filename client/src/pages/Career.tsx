...
import { completeLevel, loadCareerSaves, awardBTC } from '../utils/careerManager'
...
interface CarControllerProps {
  ...
  CarModel?: React.ComponentType<{ scale: number; rotation: [number, number, number] }>
  carRef?: React.RefObject<THREE.Group>
}

// Camera follow component
function CameraFollow({ target, levelNumber }: { target: React.RefObject<THREE.Group | null>, levelNumber: number }) {
  ...
  useFrame(() => {
    ...
    // Add 17 degrees clockwise rotation for career/1 (subtract in radians = clockwise)
    const cameraRotationOffset = levelNumber === 1 ? -1 * Math.PI / 180 : -17 * Math.PI / 180
    const adjustedRotation = carRotation + cameraRotationOffset

    // Position camera close behind and low (intimate rear view)
    targetPosition.current.x = carPosition.x - Math.sin(adjustedRotation) * cameraOffset.current.z
    targetPosition.current.y = carPosition.y + cameraOffset.current.y
    targetPosition.current.z = carPosition.z - Math.cos(adjustedRotation) * cameraOffset.current.z
    ...
    // Look at the car
    lookAtPosition.current.x = carPosition.x - Math.sin(adjustedRotation) * 0.1
    lookAtPosition.current.y = carPosition.y + 0.05
    lookAtPosition.current.z = carPosition.z - Math.cos(adjustedRotation) * 0.1
...
// AI Car that goes straight with varying speed
function AICar({ position: initialPosition, rotation: initialRotation, scale, startDelay = 3000, CarModel = Car3Model, onPositionChange }: { position: [number, number, number], rotation: number, scale: number, startDelay?: number, CarModel?: React.ComponentType<{ scale: number; rotation: [number, number, number] }>, onPositionChange?: (position: THREE.Vector3) => void }) {
  const carRef = useRef<THREE.Group>(null)
  const [raceStarted, setRaceStarted] = useState(false)
  const speedVariation = useRef<number>(1.0) // Speed multiplier
  const speedChangeTimer = useRef<number>(0)
  ...
  useFrame((_state, delta) => {
    ...
    // Vary speed every 2-4 seconds randomly (like NPC racer)
    speedChangeTimer.current += delta
    if (speedChangeTimer.current > 2 + Math.random() * 2) {
      // Speed varies between 0.7x and 1.3x of normal speed
      speedVariation.current = 0.7 + Math.random() * 0.6
      speedChangeTimer.current = 0
    }

    // AI always goes forward, never turns
    const controls = {
      forward: true,
      backward: false,
      left: false,
    ...
    // Update physics
    carState.current = updateCarPhysics(carState.current, controls, deltaMultiplier * speedVariation.current)
  ...
  // Car3Model rotation adjustment - rotate to face forward + 17 degrees anticlockwise
  const getCarRotationAdjustment = () => {
    return initialRotation + Math.PI + (17 * Math.PI / 180)
  }

  return (
    <group ref={carRef}>
      <CarModel scale={scale} rotation={[0, getCarRotationAdjustment(), 0]} />
    </group>
  )
}

function CarController({ position: initialPosition, rotation: initialRotation, scale, onPositionChange, onRotationChange, onSpeedChange, onTireMarksUpdate, reachedTarget, CarModel = Car1Model, carRef: externalCarRef }: CarControllerProps) {
  ...
  // Different car models need different rotation adjustments due to their model orientation
  const getCarRotationAdjustment = () => {
    // Car1Model (Pontiac) - rotated anticlockwise 90 degrees
    if (CarModel === Car1Model) {
      return initialRotation[1] + 2 * Math.PI - Math.PI / 2
    }
    // Car5Model - rotate 90 degrees clockwise + 17 degrees clockwise
    if (CarModel === Car5Model) {
      return initialRotation[1] + 2 * Math.PI - Math.PI / 2 - Math.PI / 2 - (1 * Math.PI / 180)
    }
    // Default fallback
    return initialRotation[1] + 2 * Math.PI - Math.PI / 2
  }
  ...
  return (
    <>
      <group ref={carRef}>
        {/* Rotate car with model-specific adjustment */}
        <CarModel scale={scale} rotation={[initialRotation[0], getCarRotationAdjustment(), initialRotation[2]]} />
      </group>
    </>
  )
}
...
function Career() {
  ...
  // Contract hooks for start/end game
  const { startGame, isLoading: isStartingGame } = useStartGame()
  const { endGame, isLoading: isEndingGame } = useEndGame()
  ...
  // Race state for level 1
  const [aiCarPosition, setAiCarPosition] = useState<THREE.Vector3 | null>(null)
  const [raceFinished, setRaceFinished] = useState<boolean>(false)
  const [raceWinner, setRaceWinner] = useState<'player' | 'shamlin' | null>(null)
  const [showRestartOption, setShowRestartOption] = useState<boolean>(false)
  ...
  // Call start game contract when career/0 or career/1 starts
  useEffect(() => {
    if (levelNumber === 0 || levelNumber === 1) {
      console.log(`[CONTRACT] Starting game for career/${levelNumber}...`)

      // Add delay to ensure any previous endGame has completed
      const startTimer = setTimeout(() => {
        startGame().then((result) => {
          if (result.success) {
            console.log(`[CONTRACT] Game started successfully for career/${levelNumber}`, result.transactionHash)
          } else {
            console.error(`[CONTRACT] Failed to start game for career/${levelNumber}:`, result.error)
          }
        })
      }, 500) // 500ms delay to ensure endGame transaction completes

      return () => clearTimeout(startTimer)
    }
  }, [levelNumber, startGame])
  ...
  // Check if player reached target zone (for level 0 and level 1)
  useEffect(() => {
    if (reachedTarget || raceFinished) return

    // Level 1 race finish line
    if (levelNumber === 1) {
      const finishX = 903.9
      const finishZ = 229.5
      const finishRange = 5
      ...
      const playerFinished =
        currentPosition.x >= finishX - finishRange &&
        currentPosition.x <= finishX + finishRange &&
        currentPosition.z >= finishZ - finishRange &&
        currentPosition.z <= finishZ + finishRange

      // Check if AI finished
      const aiFinished = aiCarPosition &&
        aiCarPosition.x >= finishX - finishRange &&
        aiCarPosition.x <= finishX + finishRange &&
        aiCarPosition.z >= finishZ - finishRange &&
        aiCarPosition.z <= finishZ + finishRange

      if (playerFinished && !raceFinished) {
        console.log('[RACE] Player finished first!')
        setRaceFinished(true)
        setRaceWinner('player')
        setReachedTarget(true)

        // Award 200 BTC for winning first blacklist race
        if (playerData && playerData.username) {
          awardBTC(playerData.username, 200)
          completeLevel(playerData.username, levelNumber, 0)
        }

        // Call end game contract for career/1
        console.log('[CONTRACT] Ending game for career/1 (won race)...')
        endGame().then((result) => {
          if (result.success) {
            console.log('[CONTRACT] Game ended successfully for career/1', result.transactionHash)
          } else {
            console.error('[CONTRACT] Failed to end game for career/1:', result.error)
          }
        })
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            navigate('/garage', { state: { playerData, wonRace: true } })
          }, 1000)
        }, 3000)
      } else if (aiFinished && !raceFinished) {
        console.log('[RACE] Shamlin finished first!')
        setRaceFinished(true)
        setRaceWinner('shamlin')
      ...
      if (isInTargetZone && !reachedTarget) {
        ...
        // Call end game contract for career/0
        console.log('[CONTRACT] Ending game for career/0...')
        endGame().then((result) => {
          if (result.success) {
            console.log('[CONTRACT] Game ended successfully for career/0', result.transactionHash)
          } else {
            console.error('[CONTRACT] Failed to end game for career/0:', result.error)
          }
        })
  ...
  }, [currentPosition, aiCarPosition, reachedTarget, raceFinished, playerData, levelNumber, navigate, coinsCollected, endGame])

  // Handle M key to toggle full map and R key to restart race
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Restart race on R key press (only if lost)
      if (e.key.toLowerCase() === 'r' && showRestartOption && levelNumber === 1) {
        console.log('[RACE] Restarting race...')
  ...
  }, [currentPosition, showRestartOption, levelNumber])
    ...
    <div style={{
      ...
      >
        ...
        <Suspense fallback={<Loader />}>
          ...
          {/* Player car with WASD controls */}
          <CarController
            key={`car-${spawnPosition.join('-')}`}
            position={spawnPosition}
            scale={levelNumber === 1 ? 1.1 : 0.6}
            ...
            CarModel={levelNumber === 1 ? Car5Model : Car1Model}
            carRef={playerCarRef}
          />

          {/* Camera follows the player car */}
          <CameraFollow target={playerCarRef} levelNumber={levelNumber} />

          {/* AI Car for level 1 only */}
          {levelNumber === 1 && (
            <AICar
              position={[846.1, 2.0, 414.7]}
              rotation={-18 * Math.PI / 180}
              scale={1.1}
              startDelay={3000}
              CarModel={Car3Model}
              onPositionChange={setAiCarPosition}
            />
          )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          {Math.round((currentRotation * 180 / Math.PI) % 360)}°
        </div>
      </div>
      ...
      {/* Race result overlay for level 1 */}
      {levelNumber === 1 && raceFinished && !showBlackScreen && (
        <div style={{
          position: 'absolute',
          top: '50%',
          left: '50%',
          transform: 'translate(-50%, -50%)',
          zIndex: 500,
          backgroundColor: raceWinner === 'player' ? 'rgba(0, 200, 0, 0.95)' : 'rgba(200, 0, 0, 0.95)',
          padding: '60px 80px',
          borderRadius: '20px',
          border: `5px solid ${raceWinner === 'player' ? '#00ff00' : '#ff0000'}`,
          boxShadow: `0 0 40px ${raceWinner === 'player' ? 'rgba(0, 255, 0, 0.8)' : 'rgba(255, 0, 0, 0.8)'}`,
          textAlign: 'center',
        }}>
          <div style={{
            color: '#ffffff',
            ...
            {raceWinner === 'player' ? 'YOU WIN!' : 'YOU LOSE!'}
          </div>
          <div style={{
            color: '#ffffff',
            fontSize: '24px',
            ...
            {raceWinner === 'player'
              ? 'You beat Shamlin to the finish line!'
              : 'Shamlin reached the finish line first!'}
          </div>
          {raceWinner === 'shamlin' && (
            <div style={{
              color: '#ffffff',
              ...
              Press <span style={{ fontWeight: 'bold', color: '#ffff00' }}>R</span> to restart race
            </div>
          )}
        </div>
      )}

      {/* Transaction Debug Overlay */}
      {showTransactionPopup && (
        <div style={{
          position: 'absolute',
          top: '20px',
          right: '20px',
          backgroundColor: 'rgba(0, 0, 0, 0.95)',
          border: transactionError ? '3px solid #ff4444' : '3px solid #00ff88',
          borderRadius: '15px',
          padding: '20px 30px',
          zIndex: 999,
          minWidth: '400px',
          maxWidth: '500px',
          fontFamily: 'Arial, Helvetica, sans-serif',
          boxShadow: transactionError ? '0 0 30px rgba(255, 68, 68, 0.5)' : '0 0 30px rgba(0, 255, 136, 0.5)',
        }}>
          <div style={{
            color: transactionError ? '#ff4444' : '#00ff88',
            fontSize: '20px',
            fontWeight: 'bold',
            marginBottom: '15px',
            textTransform: 'uppercase',
            ...
            {transactionError ? '❌ Transaction Failed' : isProcessingTransaction ? '⏳ Processing Transaction' : '✅ Transaction Confirmed'}
          </div>
          ...
          {lastBoundaryCrossing && (
            <div style={{
              ...
              <div style={{ color: '#cccccc', fontSize: '13px', lineHeight: '1.6' }}>
                <div>Position: X: {lastBoundaryCrossing.x.toFixed(2)}, Z: {lastBoundaryCrossing.z.toFixed(2)}</div>
                <div>Delta: ΔX: {lastBoundaryCrossing.deltaX}, ΔZ: {lastBoundaryCrossing.deltaY}</div>
                <div>Direction: {lastBoundaryCrossing.deltaX > 0 ? 'East' : lastBoundaryCrossing.deltaX < 0 ? 'West' : ''} {lastBoundaryCrossing.deltaY > 0 ? 'South' : lastBoundaryCrossing.deltaY < 0 ? 'North' : ''}</div>
              </div>
            </div>
          )}
          ...
          {lastTransactionHash && !transactionError && (
            <div style={{
              ...
              <div style={{ color: '#00ff88', fontSize: '12px', fontWeight: 'bold', marginBottom: '6px' }}>
                Transaction Hash:
              </div>
              <div style={{
                color: '#ffffff',
              ...
              <a
                href={`https://sepolia.voyager.online/tx/${lastTransactionHash}`}
                target="_blank"
          ...
          {!isProcessingTransaction && (
            <button
              ...
              style={{
                backgroundColor: transactionError ? '#ff4444' : '#00ff88',
                color: '#000000',
                border: 'none',
                padding: '10px 20px',
                borderRadius: '8px',
